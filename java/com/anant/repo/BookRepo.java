...
public class BookRepo {
	...
	public byte[] getBookContent(String name) {
	 ...
	 public List<Book> getRandomBooks() {
	        Session session = entityManager.unwrap(Session.class);
	        return session.createQuery("FROM Book ORDER BY RAND()", Book.class).setMaxResults(4).getResultList();
	    }
	 
	 public List<Book> getBooksByCategory(String category) {
	        Session session = entityManager.unwrap(Session.class);
	        return session.createQuery("FROM Book WHERE TRIM(LOWER(category)) = TRIM(LOWER(:category))", Book.class)
